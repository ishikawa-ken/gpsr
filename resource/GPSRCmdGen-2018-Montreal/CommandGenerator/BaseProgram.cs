...
namespace RoboCup.AtHome.CommandGenerator
{
	...
	public abstract class BaseProgram
	{
		...
		protected virtual void PrintTask(Task task)
		{
			if (task == null)
				return;
			string sTask = task.ToString().Trim();  // taskを文字列に変換して文頭と文末の空白文字を削除
			if (sTask.Length < 1)
				return;
			...
			Console.WriteLine(pad);     // ===を出力
			Console.WriteLine();

			// Prints task string and metadata
			sTask = sTask.Substring(0, 1).ToUpper() + sTask.Substring(1);   // sTaskの先頭を大文字にする
			File.AppendAllText(@"test.txt",  sTask  + Environment.NewLine);     // ファイルに書き込み
			//File.AppendAllText(@"test.txt", "<Order>" + sTask + "</Order>" + Environment.NewLine);     // ファイルに書き込み
			do
			{
				...
				Console.WriteLine(sTask.Substring(0, cut));       // 質問文を出力
				sTask = sTask.Substring(cut).Trim();
			} while (!String.IsNullOrEmpty(sTask));
			PrintTaskMetadata(task);        // 回答付きの質問文出力関数へ
			Console.WriteLine();
			// Prints another line
			Console.WriteLine(pad);     // ===を出力
		...
		protected void PrintMetadata(Token token, List<string> remarks)
		{
			...
			foreach (string md in token.Metadata){
				...
				//File.AppendAllText(@"test.txt", "\t" + "<answer>" + md + "</answer>" + Environment.NewLine);   // ファイルに出力
